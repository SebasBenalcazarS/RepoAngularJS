...
import ec.com.smx.sic.cliente.mdl.dto.id.asistentecompras.DetalleListaRecetaID;
...
@Entity
@Table(name="SCSACTDETLISREC")
public class DetalleListaRecetaDTO extends SimpleAuditDTO {

	@EmbeddedId
	private DetalleListaRecetaID id = new DetalleListaRecetaID();
	...
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumns({
		@JoinColumn(name = "CODIGOTIPODETALLE", referencedColumnName = "CODIGOCATALOGOTIPO", insertable = false, updatable = false),
		@JoinColumn(name = "VALORTIPODETALLE", referencedColumnName = "CODIGOCATALOGOVALOR", insertable = false, updatable = false)})
	...
	public DetalleListaRecetaID getId() {
		return id;
	}

	public void setId(DetalleListaRecetaID id) {
		this.id = id;
	}
	...
	public void setDescripcion(String descripcion) {
		if(descripcion!= null && descripcion.length()>199){
			this.descripcion = descripcion.substring(0, 199);
		}else{
	...
	public void setNotas(String notas) {
		if(notas!= null && notas.length()>199){
			this.notas = notas.substring(0, 199);
		}else{
			this.notas=notas;
		}
	}
